...
import { NormalizedPlugin, PluginJson, Author, HooksConfig, McpServersConfig } from './types';
 ...
 */
export function reverseTransform(normalized: NormalizedPlugin): PluginJson {
  const official: PluginJson = {};
 ...
 */
function removeEmptyAuthorFields(author: Author): Partial<Author> | undefined {
 ...
 */
function groupHooksByEvent(hooks: any[]): HooksConfig {
  ...
  for (const [event, eventHooks] of Object.entries(eventGroups)) {
    const matcherGroups: Record<string, any[]> = {};
 ...
 */
function mcpsToObject(mcps: any[]): McpServersConfig {
