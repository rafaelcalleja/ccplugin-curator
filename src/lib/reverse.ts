...
/**
 * Reverse transformation: Normalized → Official Plugin Format
 *
 * Transforms internal normalized format back to Claude Code official plugin.json format
 *
 * Based on docs/spec/006-reverse-transformation-rules.md
 */
export function reverseTransform(normalized: NormalizedPlugin): any {
  const output: any = {};
  ...
  if (normalized.commands.length > 0) {
    output.commands = normalized.commands.map(addDotSlash);
  }

  // 4. Agents (only if non-empty, add "./" prefix)
  if (normalized.agents.length > 0) {
    output.agents = normalized.agents.map(addDotSlash);
  }

  // 5. Skills (only if non-empty, add "./" prefix)
  if (normalized.skills.length > 0) {
    output.skills = normalized.skills.map(addDotSlash);
  }
 ...
 */
function removeEmptyFields(author: { name: string; email: string; url: string }): any {
  const result: any = {};
 ...
 * Input: [{ event, type, command, matcher?, ... }]
 * Output: { "EventName": [{ matcher?, hooks: [...] }] }
 */
function groupHooksByEvent(hooks: any[]): Record<string, any[]> {
  ...
  for (const hook of hooks) {
    ...
    grouped[event].get(matcher)!.push(hookConfig);
  }
...
/**
 * Convert MCPs array to object
 * Input: [{ name, command, args, env, ... }]
 * Output: { "serverName": { command, args, env, ... } }
 */
