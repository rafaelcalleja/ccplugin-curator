 ...
 */
export function toOfficialFormat(normalized: NormalizedPlugin): any {
  const official: any = {
  ...
  if (author && (author.name || author.email || author.url)) {
    official.author = {};
    if (author.name) official.author.name = author.name;
    if (author.email) official.author.email = author.email;
    if (author.url) official.author.url = author.url;
  }
  ...
  if (normalized.commands && normalized.commands.length > 0) {
    official.commands = normalized.commands.map((cmd: string) => `./${cmd}`);
  }

  if (normalized.agents && normalized.agents.length > 0) {
    official.agents = normalized.agents.map((agent: string) => `./${agent}`);
  }

  if (normalized.skills && normalized.skills.length > 0) {
    official.skills = normalized.skills.map((skill: string) => `./${skill}`);
  }
 ...
 */
export function groupHooksByEvent(hooks: Array<{
  event: string;
  type: 'command';
  command: string;
  matcher?: string;
  ...
  for (const hook of hooks) {
    ...
    let group = grouped[event].find((g: any) => {
 ...
 */
export function mcpsToObject(mcps: Array<{
  name: string;
  command: string;
  args?: string[];
 ...
 */
export function omitDefaults(plugin: any): any {
  const cleaned: any = { ...plugin };
  ...
  if (cleaned.repository === '') delete cleaned.repository;
  if (cleaned.license === '') delete cleaned.license;
