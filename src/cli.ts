...
program
  ...
  .action(async (pluginFolder: string, options) => {
    try {
 ...
 */
function scanPlugins(baseDir: string): NormalizedPlugin[] {
 ...
 */
function showSuccessMessage(outputDir: string, pluginName: string, plugins: NormalizedPlugin[]): void {
