...
import type { Hook, Mcp } from './types/index.js';
...
export async function normalizePlugin(
  ...
  // Normalize commands
  normalized.commands = normalizeComponentPaths(
    pluginJson.commands,
    discovered.commands,
    sourceDir
  );
  ...
  // Normalize agents
  normalized.agents = normalizeComponentPaths(
    pluginJson.agents,
    discovered.agents,
    sourceDir
  );
    ...
    if (typeof pluginJson.hooks === 'string') {
      ...
      if (fs.existsSync(hooksPath)) {
        try {
          ...
          (normalized as any).hooks = normalizeHooks(hooksConfig);
        } catch (error) {
          console.warn(`Failed to load hooks from ${pluginJson.hooks}:`, error);
        }
      }
    } else if (typeof pluginJson.hooks === 'object') {
      (normalized as any).hooks = normalizeHooks(pluginJson.hooks);
    }
  } else if (discovered.hooks) {
    (normalized as any).hooks = normalizeHooks(discovered.hooks);
  }
    ...
    if (typeof pluginJson.mcpServers === 'string') {
      ...
      if (fs.existsSync(mcpPath)) {
        try {
          ...
          (normalized as any).mcps = normalizeMcps(mcpConfig);
        } catch (error) {
          console.warn(`Failed to load MCPs from ${pluginJson.mcpServers}:`, error);
        }
      }
    } else if (typeof pluginJson.mcpServers === 'object') {
      (normalized as any).mcps = normalizeMcps(pluginJson.mcpServers);
    }
  } else if (discovered.mcps) {
    (normalized as any).mcps = normalizeMcps(discovered.mcps);
  }
 ...
 */
function normalizeHooks(hooksConfig: Record<string, any>): Hook[] {
  const hooks: Hook[] = [];
 ...
 */
function normalizeMcps(mcpConfig: Record<string, any>): Mcp[] {
  const mcps: Mcp[] = [];
