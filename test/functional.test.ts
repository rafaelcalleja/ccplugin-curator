...
async function runTests() {
  ...
  try {
    const testPluginsDir = path.join(projectRoot, 'test-plugins');
    console.log('  Scanning directory:', testPluginsDir);
    const plugins = await scanAndNormalizePlugins(testPluginsDir);
  ...
  try {
    ...
    const selections: SelectionState = {
      'test-plugin': {
        ...
        hooks: new Set(plugin.hooks.map((_: any, i: number) => i)), // Select all hooks
        mcps: new Set(plugin.mcps.map((_: any, i: number) => i)), // Select all MCPs
      },
    };
