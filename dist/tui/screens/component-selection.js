...
exports.showComponentSelection = showComponentSelection;
const blessed = __importStar(require("blessed"));
const state_1 = require("../state");
/**
 * Shows component selection TUI and returns user's selections
 */
async function showComponentSelection(plugins) {
    return new Promise((resolve) => {
        const state = (0, state_1.createState)(plugins);
        ...
        let currentPanel = 'plugins';
        /**
         * Updates the components list based on selected plugin
         */
        function updateComponentsList() {
            if (state.focus.pluginIndex >= plugins.length)
                return;
        ...
        function updatePreview() {
            const preview = (0, state_1.generatePreview)(state);
        ...
        function toggleCurrentComponent() {
            ...
            if (!plugin)
                return;
            const items = componentsPanel.items;
            const selectedIndex = componentsPanel.selected;
            const item = items[selectedIndex];
            if (!item || item.startsWith('{bold}'))
                return; // Skip headers
            ...
            for (let i = 0; i <= selectedIndex; i++) {
                ...
                if (line.includes('{bold}Commands:{/bold}'))
                    currentType = 'commands';
                else if (line.includes('{bold}Agents:{/bold}'))
                    currentType = 'agents';
                else if (line.includes('{bold}Skills:{/bold}'))
                    currentType = 'skills';
                else if (line.includes('{bold}Hooks:{/bold}'))
                    currentType = 'hooks';
                else if (line.includes('{bold}MCPs:{/bold}'))
                    currentType = 'mcps';
                else if (!line.startsWith('{bold}')) {
                    if (currentType === 'commands')
                        cmdCount++;
                    else if (currentType === 'agents')
                        agentCount++;
        ...
        screen.key(['a', 'A'], () => {
            const plugin = plugins[state.focus.pluginIndex];
            if (plugin) {
                (0, state_1.selectAll)(state, plugin.name);
                updateComponentsList();
                updatePreview();
            }
        });
        ...
        screen.key(['n', 'N'], () => {
            const plugin = plugins[state.focus.pluginIndex];
            if (plugin) {
                (0, state_1.deselectAll)(state, plugin.name);
                updateComponentsList();
                updatePreview();
            }
        });
...
//# sourceMappingURL=component-selection.js.map