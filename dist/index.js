...
async function main() {
    try {
        ...
        if (plugins.length === 0) {
            console.error(chalk.red('✗ No plugins found. Ensure you have .claude-plugin/plugin.json files.'));
            process.exit(1);
        }
        ...
        for (const plugin of plugins) {
            const sourceDir = plugin.__sourceDir;
        ...
        const exitCode = await new Promise((resolve) => {
            const { unmount, waitUntilExit } = render(React.createElement(PluginCurator, {
                ...
                onSave: async (selections) => {
                    ...
                    const hasSelection = selections.some((s) => s.commands.length > 0 ||
                        s.agents.length > 0 ||
                        s.skills.length > 0 ||
                        s.hooks.length > 0 ||
                        s.mcps.length > 0);
